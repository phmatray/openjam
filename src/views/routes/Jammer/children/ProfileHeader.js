      ...
      <GridColumn width={16} color="blue">
        ...
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <div style={{ display: 'flex', margin: 'auto 0', padding: '5px' }}>
            {isEmpty(profile.website) ? null : <SocialIcon href={profile.website} />}

            {isEmpty(profile.social && profile.social.soundcloud) ? null : (
              <SocialIcon href={profile.social.soundcloud} />
            )}

            {isEmpty(profile.social && profile.social.twitter) ? null : (
              <SocialIcon href={profile.social.twitter} />
            )}

            {isEmpty(profile.social && profile.social.facebook) ? null : (
              <SocialIcon href={profile.social.facebook} />
            )}

            {isEmpty(profile.social && profile.social.linkedin) ? null : (
              <SocialIcon href={profile.social.linkedin} />
            )}

            {isEmpty(profile.social && profile.social.instagram) ? null : (
              <SocialIcon href={profile.social.instagram} />
            )}

            {isEmpty(profile.social && profile.social.youtube) ? null : (
              <SocialIcon href={profile.social.youtube} />
            )}
          </div>
        </div>
      </GridColumn>
